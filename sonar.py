...
def drawBoard(board):
    ...
    for row in range(15):
        # Single-digit numbers need to be padded with a extra space.
        if row < 10:
            extraSpace = ' '
        else:
            extraSpace = ''
...
def getRandomChests(numChests):
    # Create a list of chest data structures (two-item list of x, y int coordinates).
    chests = []
...
def isOnBoard(x, y):
    # Return True if the coordinates are on board; otherwise, return False.
...
def makeMove(board, chests, x, y):
    ...
    # Return False if this is a invalid move.
...
def enterPlayerMove(previousMoves):
    ...
    print('Where do you want to drop the next sonar devise? (o-59 0-14) (or type quit)')
    while True:
        move = input()
        if move.lower()=='quit':
...
sonar divices update to show how far away the next nearest chest is. The chests
...
print ('S O N A R!')
print ()
print ('Would you like to view the instruction? (yes/no)')
